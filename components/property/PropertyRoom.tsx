...
import { CINEMA, CONTENT, EASE, STAGGER } from '@/motion/config';
...
export default function PropertyRoom({ location }: { location: Location }) {
  ...
  useMotionEffect(root, () => {
    ...
    // A specification sheet: the rows should arrive quickly and get out of the
    // way, so this is the tightest stagger on the site.
    gsap.from('.pr-row', {
      y: 12,
      opacity: 0,
      ...
      scrollTrigger: { trigger: '.pr-spec', start: 'top 82%', once: true },
    });
  });
  ...
  return (
    <section ref={root} id="room" className="gutter relative bg-void py-20 md:py-28">
      ...
      {gallery.length ? (
        <>
          <div className="mt-16 grid grid-cols-2 gap-3 md:mt-20 md:grid-cols-4 md:gap-4">
            {gallery.map((slug, i) => (
              // `crop` is the quietest of the four signature reveals — right
              // for supporting imagery that should not announce itself (§10).
              <RevealImage key={slug} style="crop" delay={i * 0.06} scaleFrom={1.08}>
                <Frame
          ...
          {location.imagesArePlaceholder ? (
            <p className="pr-row mt-4 text-xs text-smoke">
        ...
        <div className="pr-note mt-16 flex flex-col items-center gap-5 border border-hairline bg-pitch px-6 py-16 text-center md:mt-20">
          <Emblem sizes="96px" className="w-20 opacity-25 md:w-24" />
